@extends('layouts.app')
@section('title',ucfirst(config('settings.document_label_plural'))." Danh sách")
@section('css')
    <style type="text/css">
        .bg-folder-shaper {
            width: 100%;
            ...
            <div class="box-header">
                <div class="form-group hidden visible-xs">
                    <button type="button" class="btn btn-default btn-block" data-toggle="collapse"
                            data-target="#filterForm"><i class="fa fa-filter"></i> Lọc
                    </button>
                </div>
                {!! Form::model(request()->all(), ['method'=>'get','class'=>'form-inline visible hidden-xs','id'=>'filterForm']) !!}
                <div class="form-group">
                    <label for="search" class="sr-only">Tìm kiếm</label>
                    {!! Form::text('search',null,['class'=>'form-control input-sm','placeholder'=>'Tìm kiếm...']) !!}
                </div>
                <div class="form-group">
                                ...
                                <div class="widget-user-header bg-gray bg-folder-shaper no-padding">
                                        ...
                                        <div class="box-tools pull-right">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-left" role="menu">
                                                    <li><a href="{{route('documents.show',$document->id)}}">Hiển thị</a>
                                                    </li>
                                                    @can('edit',$document)
                                                        <li><a href="{{route('documents.edit',$document->id)}}">Sửa</a>
                                                        </li>
                                                    @endcan
                                                    @can('delete',$document)
                                                        <li>
                                    ...
                                    @foreach ($document->tags as $tag)
                                        ...
                                        <h5 class="widget-user-desc" style="font-size: 12px"><span data-toggle="tooltip"
                                            ...
                                            {!! $document->isVerified ? '<i title="Đã xác minh" data-toggle="tooltip" class="fa fa-check-circle" style="color: #388E3C;"></i>':'<i title="Chưa được xác minh" data-toggle="tooltip" class="fa fa-remove" style="color: #f44336;"></i>' !!}
                                        </span></h5>
                                    </a>
                                </div>
                            </div>
