@extends('layouts.app')
@section('title',"Tải lên ".ucfirst(config('settings.file_label_plural')))
@section('css')
    <style>
        ...
        <div class="box box-primary file-row file-row-@{{index}}">
            <div class="box-header">
                <div class="pull-right box-tools">
                    <button onclick="removeRow(this)" type="button" class="btn btn-danger btn-sm" title="Gỡ">
    ...
    <script>
        ...
        function showErrors(errors) {
            ...
            $('form#frmUploads').before('<div id="mainErrors" class="alert alert-danger alert-dismissible">\n' +
                ...
                '<strong>Dữ liệu của bạn có lỗi, vui lòng xem lại và sửa các lỗi này.</strong>\n' +
                '</div>');
            window.scrollTo(0, 0);
        ...
        function submitForm(frm) {
            ...
            $(frm).find("button[type='submit']").html('<i class="fa fa-spinner fa-spin"></i> Đang tải...').attr("disabled", true);
...
@section('content')
    <section class="content-header">
        <h1>
            Tải lên {{ucfirst(config('settings.file_label_plural'))}} - <small>{{$document->name}}</small>
    ...
    <div class="content">
        ...
        @if(!count($fileTypes))
            <div class="alert alert-warning alert-dismissible">
                <button class="close" data-dismiss="alert" aria-label="close">&times;</button>
                <strong>Không có {{config('settings.file_label_singular')." type"}},</strong> Vui lòng thêm ít nhất một.
                <a href="{{route('fileTypes.create')}}">Thêm {{config('settings.file_label_singular')." type"}}</a>
            </div>
        @endif
        ...
        <div style="text-align: center">
            <button type="button" class="btn btn-info btn-sm" onclick="addRow()"><i class="fa fa-plus"></i>
                Thêm {{ucfirst(config('settings.file_label_singular'))}}
            </button>
            <button type="submit" class="btn btn-primary btn-sm"><i class="fa fa-save"></i> Tải lên</button>
        </div>
        {!! Form::close() !!}
    </div>
@endsection
