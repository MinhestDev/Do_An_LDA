@extends('layouts.app')
@section('title',"Hiển thị ".ucfirst(config('settings.document_label_singular')))
@section('css')
    <style>
        .box.custom-box {
                            ...
                            <div class="form-group">
                                <a href="{{\Illuminate\Support\Str::finish(route('files.showfile',['dir'=>'original']),"/")}}@{{file}}?force=true"
                                   download class="btn btn-primary"><i
                                        class="fa fa-download"></i> Tải xuống bản gốc
                                </a>
                            </div>
                            <div class="form-group">
                                <label>{{ucfirst(config('settings.file_label_singular'))." Loại"}}</label>
                                <p>@{{file_type.name}}</p>
                            </div>
                            <div class="form-group">
                                <label>Đã tải lên bởi:</label>
                                <p>
                                    @{{created_by.name}}
                                </p>
                            </div>
                            <div class="form-group">
                                <label>Tải lên lúc:</label>
        ...
        <h1 class="pull-right" style="margin-bottom: 5px;">
            <div class="dropdown" style="display: inline-block">
                <button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown"><i
                        class="fa fa-download"></i> Tải bản ZIP
                    <span class="caret"></span></button>
                <ul class="dropdown-menu">
                    <li>
                        <a href="{{route('files.downloadZip',['dir'=>'all','id'=>$document->id])}}">Tất cả </a>
                    </li>
                    <li>
                        <a href="{{route('files.downloadZip',['dir'=>'original','id'=>$document->id])}}">Bản gốc</a>
                    </li>
        ...
        <div class="row">
            <div class="col-sm-3">
                <div class="box box-primary">
                    <div class="box-body">
                        <div class="form-group">
                            <label>{{ucfirst(config('settings.document_label_singular'))}} Tên:</label>
                            <p>{{$document->name}}</p>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label>Đã tạo bởi:</label> {{$document->createdBy->name}}
                        </div>
                        <div class="form-group">
                            <label>Đã tạo lúc:</label>
                            <p>{!! formatDateTime($document->created_at) !!} <br>
                                ({{\Carbon\Carbon::parse($document->created_at)->diffForHumans()}})
                            </p>
                        </div>
                        <div class="form-group">
                            <label>Đã cập nhật lần cuối:</label>
                    ...
                    <ul class="nav nav-tabs">
                        ...
                        @can('verify', $document)
                            <li class=""><a href="#tab_verification" data-toggle="tab"
                                            aria-expanded="false">Xác minh</a></li>
                        @endcan
                        <li class=""><a href="#tab_activity" data-toggle="tab" aria-expanded="false">Hành động</a></li>
                        @can('user manage permission')
                            <li class=""><a href="#tab_permissions" data-toggle="tab"
                                            aria-expanded="false">Cho phép</a>
                            </li>
                        @endcan
                    </ul>
                        ...
                        <div class="tab-pane active" id="tab_files">
                            @if (config('settings.show_missing_files_errors')=='true' && $document->status!=config('constants.STATUS.APPROVED') && count($missigDocMsgs)!=0)
                                <div class="alert alert-danger fade in alert-dismissible">
                                    ...
                                    <strong>Đang theo dõi {{ucfirst(config('settings.file_label_plural'))}} Đang xóa:</strong>
                            ...
                            @can('update', [$document, $document->tags->pluck('id')])
                                <a href="{{route('documents.files.create',$document->id)}}"
                                   class="btn btn-primary btn-sm"><i class="fa fa-plus"></i>
                                    Thêm {{ucfirst(config('settings.file_label_plural'))}}</a>
                            @endcan
                        </div>
                        @can('verify', $document)
                            <div class="tab-pane" id="tab_verification">
                                ...
                                @else
                                    ...
                                    <div class="form-group">
                                        Người xác minh: <b>{{$document->verifiedBy->name}}</b>
                                    </div>
                                    <div class="form-gorup">
                                        Đã xác minh lúc: <b>{{formatDateTime($document->verified_at)}}</b>
                            ...
                            <div class="tab-pane" id="tab_permissions">
                                <div>
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    ...
                                                    <h4 class="modal-title">Nhận quyền</h4>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-sm-12">
                                                        ...
                                                        @foreach (config('constants.DOCUMENT_LEVEL_PERMISSIONS')  as $perm)
                                                            <div class="col-sm-6" style="margin-top: 20px;">
                                                                <label>
                                                                    ...
                                                                    đây {{config('settings.document_label_singular')}}
                                                                </label>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                        ...
                                        <tr>
                                            <th colspan="3" style="font-size: 1.8rem;">
                                                ...
                                                <button type="button" class="btn btn-primary btn-xs pull-right" data-toggle="modal"
                                                    ...
                                                    <i class="fa fa-plus"></i> Quyền mới
                                                </button>
                                            </th>
                                        </tr>
                                        <tr>
                                    ...
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th colspan="3" style="font-size: 1.8rem;">Quyền kế thừa bởi {{config('settings.tags_label_plural')}}</th>
                                        </tr>
                                        <tr>
                                    ...
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th colspan="3" style="font-size: 1.8rem;">Quyền của {{config('settings.document_label_plural')}}</th>
                                        </tr>
                                        <tr>
                                            <th>Người dùng</th>
                                            <th>Quyền</th>
